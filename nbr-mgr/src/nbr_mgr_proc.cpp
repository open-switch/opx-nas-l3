...
bool nbr_process::nbr_proc_fdb_msg(const nbr_mgr_nbr_entry_t& entry) {
    ...
    try {
        if(entry.msg_type == NBR_MGR_NBR_ADD) {
            ...
            if ((entry.mbr_if_index && (nbr_check_intf_oper_up(NBR_MGR_DEFAULT_VRF_ID, entry.mbr_if_index) == false)) ||
                (nbr_check_intf_oper_up(NBR_MGR_DEFAULT_VRF_ID, entry.if_index) == false)) {
                is_oper_up = false;
            }
        }
        //Invoking observer handlers
        ptr->for_each_nbr_list([&entry, &is_oper_up](const nbr_data *n_ptr, std::string mac) {
                               n_ptr->handle_fdb_change(entry.msg_type, entry.status, is_oper_up);
                               });
        ptr->display();
...
bool nbr_process::nbr_proc_intf_msg(nbr_mgr_intf_entry_t& intf) {
    NBR_MGR_LOG_INFO("PROC", "Intf VRF-id:%lu is-del:%d, if_index:%d, msg-type:%s "
...
bool nbr_data::handle_fdb_change(nbr_mgr_evt_type_t evt, unsigned long status, bool is_oper_up) const {
    std::string ip = nbr_ip_addr_string (m_ip_addr);
    NBR_MGR_LOG_DEBUG("PROC-NL-MSG", "Handle FDB change for %s if-index:%d evt:%d status:%d oper:%d",
                      ip.c_str(), m_ifindex, evt, status, is_oper_up);
    ...
    if ((evt == NBR_MGR_NBR_DEL) || (status & NBR_MGR_NUD_STALE)) {
        ...
        if (!(m_status & NBR_MGR_NUD_PERMANENT) &&
            ...
            !(m_status & NBR_MGR_NUD_INCOMPLETE)) {
            m_flags |= NBR_MGR_NBR_REFRESH;
            if ((evt == NBR_MGR_NBR_DEL) && (is_oper_up == false)) {
                /* Oper. down case, dont refresh the ARP/Nbr. */
                nbr_stats.oper_down_mac_trig_instant_refresh++;
                /* Since the intention here is to make the neighbor entries in kernel
                 * to go failed state on intf. oper. down, dont rate-limit,
                 * send the refresh for all neighbors as soon as possible. */
                return trigger_instant_refresh();
            }
...
bool nbr_data::handle_if_state_change(nbr_mgr_intf_entry_t& intf) {
    ...
    bool trig_nbr_resolve = false;
    if (intf.flags & NBR_MGR_INTF_ADMIN_MSG) {
        if (intf.is_admin_up) {
            trig_nbr_resolve = true;
        } else {
            m_status = NBR_MGR_NUD_NONE;
        }
    }
